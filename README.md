...
| 指令 | 功能 | 使用方式1：回复消息 | 使用方式2：直接输入 |
|------|------|-------------------|-------------------|
| `/id` | 查看用户信息 | 回复用户消息后输入 `/id` | `/id 123456789` |
| `/b` 或 `/block` | 拉黑用户 | 回复用户消息后输入 `/b` | `/b 123456789` |
| `/ub` 或 `/unblock` | 解除拉黑 | 回复用户消息后输入 `/ub` | `/ub 123456789` |
| `/bl` 或 `/blocklist` | 查看黑名单 | - | `/bl` 或 `/blocklist` |
| `/uv` 或 `/unverify` | 取消验证 | 回复用户消息后输入 `/uv` | `/uv 123456789` |
...
| 清理失效Bot | 🗑️ | 自动检测并批量删除 Token 失效的机器人 | 保持系统健康，需二次确认 |

> 详细使用说明请查看 [管理员功能指南](ADMIN_GUIDE.md)
